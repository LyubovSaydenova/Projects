...
# Загрузка данных, обработка пропусков, удаление дубликатов
...
# График частоты покупок от месяца
sns.set()
ax = sns.scatterplot(x = 'Дата', y = 'Сумма', color = "gold", data = purchase_rate)
plt.show()

# Группировка по товарам
kmeans = KMeans(n_clusters = 5, random_state = 0).fit(sum_data[['Количество', 'Сумма']])
predict = kmeans.predict(sum_data[['Количество', 'Сумма']])
sum_data['Кластер'] = predict
sum_data.head()